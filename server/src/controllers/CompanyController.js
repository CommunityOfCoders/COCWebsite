...
const createCompany = async (req, res) => {
  try {
    if (!req.body || !req.body.companyName || !req.file) {
      return res
        .status(422)
        .json({ error: "Please provide name of the company and image" });
    }
    ...
    if (companyExists) {
      return res
        .status(400)
        .json({ error: "Company with given name already exists" });
    }
    const file = req.file;
...
//       return res
//         .status(422)
//         .json({ error: "Please provide name of the company" });
//     }

//     const companyName = req.body.companyName;
//     const companyNotExists = await Company.findOne({ title: companyName });
//     if (!companyNotExists) {
//       return res
//         .status(400)
//         .json({ error: "Company with given name does not exist" });
//     }
//     // const companyDetails = {
//     //   title: companyName,
//     // };

//     await Company.findByIdAndDelete({ title: companyName }, () => {
//       if (err) {
...
const deleteCompanyById = async (req, res) => {
  try {
    ...
    if (!companyExists) {
      return res
        .status(400)
        .json({ error: "Company with given id does not exist" });
    }
